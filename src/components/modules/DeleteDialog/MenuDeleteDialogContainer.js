...
import { removeMenu, loadMenus } from '../../../modules/manage/menu';
...
const DeleteDialogContainer = ({
  ...
  const { remove, error } = useSelector(({ menu }) => ({
    remove: menu.remove,
  ...
  useEffect(() => {
    ...
    if (remove) {
      ...
      const token = localStorage.getItem('token');
      dispatch(loadMenus({ direction, page, size, token }));
      setOpen();
      history.goBack();
    }
