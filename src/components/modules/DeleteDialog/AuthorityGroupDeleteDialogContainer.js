...
const DeleteDialogContainer = ({
  ...
  const { remove, error } = useSelector(({ authorityGroup }) => ({
    remove: authorityGroup.remove,
    error: authorityGroup.removeError
  }));

  const onDelete = () => {
    const token = localStorage.getItem('token');
    const id = data.authorityGroupId;
    dispatch(removeAuthorityGroup({ id, token }));
  };
