...
const DeleteDialogContainer = ({
  ...
  const { remove, error } = useSelector(({ tagListen }) => ({
    remove: tagListen.remove,
    error: tagListen.removeError
  }));

  const onDelete = () => {
    const token = localStorage.getItem('token');
    const id = data.tagListeningId;
    dispatch(removeTagListen({ id, token }));
  };
