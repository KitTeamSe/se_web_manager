...
import { removeAccount, loadAccounts } from '../../../modules/manage/account';
...
const DeleteDialogContainer = ({
  ...
  const { remove, error } = useSelector(({ account }) => ({
    remove: account.remove,
    error: account.removeError
  }));
  ...
  const onDelete = () => {
    ...
    dispatch(removeAccount({ id, token }));
  };
